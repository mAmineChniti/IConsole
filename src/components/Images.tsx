...
export function Images() {
  ...
  const editForm = useForm<z.infer<typeof ImageUpdateRequestSchema>>({
  ...
  return (
    <div className="px-2 space-y-6 sm:px-0">
      ...
      <div className="flex flex-col gap-4 items-stretch sm:flex-row sm:items-center">
        ...
        <Dialog open={isImportDialogOpen} onOpenChange={setIsImportDialogOpen}>
          <DialogTrigger asChild>
            <Button
              variant="outline"
              className="gap-2 w-full h-10 rounded-full cursor-pointer sm:w-auto min-w-[140px]"
            >
              <Plus className="flex-shrink-0 w-4 h-4" />
              <span className="truncate">Import from URL</span>
            </Button>
          </DialogTrigger>
        ...
        <Dialog open={isImportByNameOpen} onOpenChange={setIsImportByNameOpen}>
          <DialogTrigger asChild>
            <Button
              variant="outline"
              className="gap-2 w-full h-10 rounded-full cursor-pointer sm:w-auto min-w-[140px]"
            >
              <Plus className="flex-shrink-0 w-4 h-4" />
              <span className="truncate">Import by Name</span>
            </Button>
          </DialogTrigger>
          <DialogContent className="mx-4 sm:mx-0 max-w-[calc(100vw-2rem)] sm:max-w-md bg-card text-card-foreground border border-border/50 shadow-lg left-1/2 translate-x-[-50%] rounded-2xl">
            <DialogHeader>
            ...
            <Form {...importByNameForm}>
              <form onSubmit={handleImportByName} className="space-y-4">
                <FormField
                ...
                <div className="flex flex-col gap-3 justify-end sm:flex-row">
                  <Button
                    ...
                    variant="outline"
                    className="order-2 w-full rounded-full cursor-pointer sm:order-1 sm:w-auto"
                    onClick={() => setIsImportByNameOpen(false)}
      ...
      <Dialog
        open={!!imageToDelete}
        onOpenChange={(open) => {
          if (!open) setImageToDelete(undefined);
        }}
      >
        <DialogContent className="mx-4 sm:mx-0 max-w-[calc(100vw-2rem)] sm:max-w-md bg-card text-card-foreground border border-border/50 shadow-lg left-1/2 translate-x-[-50%] rounded-2xl">
          <DialogHeader>
          ...
          <div className="space-y-3 text-sm">
            ...
            {imageToDelete && (
              <div className="py-2 px-3 text-sm rounded-md border">
                <span className="text-muted-foreground">Name:</span>{" "}
                <span className="font-medium">{imageToDelete.name}</span>
              </div>
            )}
          ...
          <div className="flex flex-col gap-3 justify-end mt-2 sm:flex-row">
            <Button
              type="button"
              variant="outline"
              className="order-2 w-full rounded-full cursor-pointer sm:order-1 sm:w-auto"
              onClick={() => setImageToDelete(undefined)}
            ...
            <Button
              type="button"
              variant="destructive"
              className="order-1 w-full text-white rounded-full cursor-pointer sm:order-2 sm:w-auto min-w-[120px]"
              onClick={() =>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        disabled={deleteMutation.isPending || !!image.protected}
                      >
