...
export default function InstanceLogs({ instance, onClose }: InstanceLogsProps) {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await InfraService.getLogs({ instance_id: instance.id });
      return response.logs.output || "No logs available";
    },
      ...
      <div className="sticky top-0 z-10 flex-none p-4 border-b bg-background/95 backdrop-blur-sm">
        <div className="flex gap-4 justify-between items-center">
          ...
          <div className="flex gap-2 items-center">
            <div
              className={cn(
                "hidden sm:flex items-center gap-1.5 px-2.5 py-1 text-xs rounded-full",
                autoRefresh
